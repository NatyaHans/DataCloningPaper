...
mcmc.default <- function(lik, x.init, nsteps, w, prior=NULL,
  ...
  if ( is.null(prior) )
    ...
    posterior <- protect(function(x) k.clone*lik(x, ...) + prior(x),
                         fail.value.default=fail.value)
  ...
  if(k.clone<0){
    stop("Provide a positive number of clones")  # Kill if number of clones isn't a positive number
  } #####ADDED HERE
...
check.par.length <- diversitree:::check.par.length
check.bounds <- diversitree:::check.bounds
get.info<- diversitree:::get.info
